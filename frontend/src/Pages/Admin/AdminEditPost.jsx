...
const AdminEditPost = () => {
  const { id } = useParams(); //라우터에서 파라미터로 넘어온 id값을 가져옴
  const navigate = useNavigate(); //수정 후 목록으로 이동하기 위해 사용
  const [formData, setFormData] = useState({
    title: "",
    content: "",
    files: [],
    fileList: [],
    existingFiles: [], //기존에 업로드된 파일들을 저장
  });
  ...
  useEffect(() => {
    ...
    const fetchPostData = async () => {
      try {
        ...
        console.log("포스트 데이터 불러오기 오류: ", error);
        navigate("/admin/posts"); //오류시 목록으로 이동
      }
    };
    fetchPostData();
  }, [id, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const postData = {
        ...
        fileUrl: [...formData.existingFiles, ...uploadedFiles], //기존 파일과 새로 업로드된 파일 합침
        currentImages: currentImages,
      };

      await axios.post(`http://localhost:3001/api/post/${id}`, postData, {
        withCredentials: true,
        headers: {
  ...
  const handleExistingFileDelete = (fileUrl) => {
    setFormData((prev) => ({
      ...prev,
      existingFiles: prev.existingFiles.filter((url) => url !== fileUrl), //기존 파일 배열에서 해당 파일 제거
    }));
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      <div className="bg-white rounded-lg shadow-lg p-4 sm:p-8">
        ...
        <form className="space-y-4 sm:space-y-8" onSubmit={handleSubmit}>
          ...
          {formData.existingFiles.length > 0 && ( //기존 파일을 가져오도록 함
            <div className="mt-4">
